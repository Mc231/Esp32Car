...
CarWebServer::CarWebServer(CarController& carController, CarApplicationConfig config, SystemMonitor&)
  : carController(carController), config(config), systemMonitor(systemMonitor), server(32231) // Initialize motorControl here
{}
...
void CarWebServer::handleReboot() {
    server.send(200, "text/plain", "Rebooting");
    carController.reboot();
}

void CarWebServer::handleGetWiFi() {
    String response = carController.getWiFiConfig();
    server.send(200, "application/json", response);
}

void CarWebServer::handleWiFiForget() {
